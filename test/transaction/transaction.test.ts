...
describe('sign transaction', function () {
  ...
  it('sign P2TR multisigAddress', async function () {
    ...
    const tapLeafScript = [
      {
        ...
        controlBlock: multisigPayment.witness![multisigPayment.witness!.length - 1]
      }
    ];
    ...
    const wallet2 = new LocalWallet(signer2.toWIF(), AddressType.P2WPKH, NetworkType.TESTNET);
    await wallet1.signPsbt(psbtB, {
      toSignInputs: [{ index: 0, publicKey: wallet1.pubkey, disableTweakSigner: true }]
    });

    await wallet2.signPsbt(psbtB, {
      toSignInputs: [{ index: 0, publicKey: wallet2.pubkey, disableTweakSigner: true }]
    });
